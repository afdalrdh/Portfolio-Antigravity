...
// api/src/services/homeService.ts
import { eq } from "drizzle-orm";
var homeService;
var init_homeService = __esm({
  "api/src/services/homeService.ts"() {
    "use strict";
    init_db();
    init_home();
...
// api/src/services/aboutService.ts
...
// api/src/services/projectService.ts
import { eq as eq4 } from "drizzle-orm";
var DEFAULT_PROJECTS, projectService;
...
var defaultBase, authBase, baseURL, auth;
var init_auth2 = __esm({
  "api/src/lib/auth.ts"() {
    "use strict";
    init_db();
    init_schema();
    defaultBase = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "https://afdalrdh.com";
    authBase = process.env.BETTER_AUTH_URL || defaultBase;
    baseURL = authBase.endsWith("/api/auth") ? authBase : `${authBase}/api/auth`;
    auth = betterAuth({
...
var router2, adminRoutes_default;
...
// api/src/routes/publicRoutes.ts
init_homeService();
init_aboutService();
init_contactService();
init_projectService();
...
app.all("/api/auth/*", async (req, res) => {
  try {
    ...
    return toNodeHandler(auth2)(req, res);
  } catch (error) {
...
app.use("/api/admin", async (req, res, next) => {
  try {
    const { requireAuth: requireAuth2 } = await Promise.resolve().then(() => (init_requireAuth(), requireAuth_exports));
    const { default: adminRoutes } = await Promise.resolve().then(() => (init_adminRoutes(), adminRoutes_exports));
    return requireAuth2(req, res, () => adminRoutes(req, res, next));
  } catch (error) {
